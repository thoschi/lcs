...
def main():
   ...
   root.title('Schulnetz-Passwort eingeben' if existing else 'Schulnetz-Login und -Passwort eingeben')
   root.resizable(False, False)
   frame = tk.Frame(root, padx=22, pady=18)
   frame.pack()
   text = ('Geben Sie genau das Passwort Ihres Schulnetz-Zugangs ein.' if existing else
   ...
   tk.Label(frame, text='Schulnetz-Passwort').grid(row=2, column=0, sticky='w', pady=4)
   ...
   button = tk.Button(frame, text='Lokales Konto einrichten', command=submit)
   button.grid(row=3, column=0, columnspan=2, sticky='e', pady=(14, 0))
